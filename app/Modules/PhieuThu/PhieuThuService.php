...
class PhieuThuService
{
  ...
  public function create(array $data)
  {
    try {
      ...
      $result = match ($data['loai_phieu_thu']) {
        ...
        default => CustomResponse::error('Loại phiếu thu không hợp lệ')
      };
  ...
  private function xuLyThanhToanNhieuDonHang(array $data)
  {
    if (empty($data['don_hang_ids'])) {
      return CustomResponse::error('Danh sách đơn hàng không được để trống');
    }
    ...
    if ($donHangs->isEmpty()) {
      return CustomResponse::error('Không tìm thấy công nợ của khách hàng này');
    }
  ...
  private function xuLyThanhToanCongNoKhachHang(array $data)
  {
    if (empty($data['khach_hang_id'])) {
      return CustomResponse::error('Khách hàng không được để trống');
    }
    ...
    if ($tongTienCanThanhToan < $data['so_tien']) {
      return CustomResponse::error('Số tiền thanh toán nhiều hơn số tiền cần thanh toán khách hàng');
    }
  ...
  private function kiemTraSoTienThanhToanHopLe($donHang, $soTienThanhToan)
  {
    ...
    if ($soTienThanhToan > $soTienCanThanhToan) {
      return CustomResponse::error('Số tiền thanh toán nhiều hơn số tiền cần thanh toán');
    }
